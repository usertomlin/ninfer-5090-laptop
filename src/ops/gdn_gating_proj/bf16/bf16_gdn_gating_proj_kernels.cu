...
void require_shape08(const Weight& w, const char* name) {
    if (w.n != k08N || w.k != k08K || w.shape[0] != k08N || w.shape[1] != k08K) {
        throw std::invalid_argument(std::string("gdn_gating_proj: ") + name +
                                    " requires contiguous BF16 [16,1024]");
    }
}
...
constexpr std::int32_t cooperative_resident_ctas_per_sm() noexcept {
    ...
    if constexpr (std::is_same_v<Geometry, Bf16Gdn27Geometry>) {
        ...
        static_assert(std::is_same_v<Geometry, Bf16Gdn35Geometry> ||
                      ...
                      std::is_same_v<Geometry, Bf16Gdn08Geometry>);
...
bool bf16_gdn_gating_proj_08_mma_split16_launch(
    Bf16GdnGatingTokenVariant variant, const Tensor& x, const Weight& a_weight,
    const Weight& b_weight, const Tensor& A_log, const Tensor& dt_bias, void* workspace,
    Tensor& g, Tensor& beta, std::int32_t multiprocessor_count, cudaStream_t stream) {
    require_shape08(a_weight, "a_weight");
    require_shape08(b_weight, "b_weight");
    return launch_bf16_prefill_mma<Bf16Gdn08Geometry, 16, 8>(
        variant, x, nullptr, 0.0F, nullptr, a_weight, b_weight, A_log, dt_bias, workspace, g, beta,
        stream, multiprocessor_count);
}

bool bf16_gdn_gating_proj_08_mma_split8_launch(
    Bf16GdnGatingTokenVariant variant, const Tensor& x, const Weight& a_weight,
    const Weight& b_weight, const Tensor& A_log, const Tensor& dt_bias, void* workspace,
    Tensor& g, Tensor& beta, std::int32_t multiprocessor_count, cudaStream_t stream) {
    require_shape08(a_weight, "a_weight");
    require_shape08(b_weight, "b_weight");
    return launch_bf16_prefill_mma<Bf16Gdn08Geometry, 8, 8>(
        variant, x, nullptr, 0.0F, nullptr, a_weight, b_weight, A_log, dt_bias, workspace, g, beta,
        stream, multiprocessor_count);
}

bool bf16_gdn_gating_proj_08_mma_split4_launch(
    Bf16GdnGatingTokenVariant variant, const Tensor& x, const Weight& a_weight,
    const Weight& b_weight, const Tensor& A_log, const Tensor& dt_bias, void* workspace,
    Tensor& g, Tensor& beta, std::int32_t multiprocessor_count, cudaStream_t stream) {
    require_shape08(a_weight, "a_weight");
    require_shape08(b_weight, "b_weight");
    return launch_bf16_prefill_mma<Bf16Gdn08Geometry, 4, 8>(
        variant, x, nullptr, 0.0F, nullptr, a_weight, b_weight, A_log, dt_bias, workspace, g, beta,
        stream, multiprocessor_count);
}

bool bf16_gdn_gating_proj_08_mma_split2_launch(
    Bf16GdnGatingTokenVariant variant, const Tensor& x, const Weight& a_weight,
    const Weight& b_weight, const Tensor& A_log, const Tensor& dt_bias, void* workspace,
    Tensor& g, Tensor& beta, std::int32_t multiprocessor_count, cudaStream_t stream) {
    require_shape08(a_weight, "a_weight");
    require_shape08(b_weight, "b_weight");
    return launch_bf16_prefill_mma<Bf16Gdn08Geometry, 2, 8>(
        variant, x, nullptr, 0.0F, nullptr, a_weight, b_weight, A_log, dt_bias, workspace, g, beta,
        stream, multiprocessor_count);
}
