...
// Qwen3.5-2B ships the same template as Qwen3.5-4B with the `enable_thinking` test inverted, which
// only changes the implicit default for an undefined option. The engine always passes the option
// explicitly, so both digests compile to the same ThinkingToggle semantics.
constexpr Sha256Digest kQwen35NonThinkingDefaultChatTemplateDigest{
...
constexpr Sha256Digest kOrnith15ChatTemplateDigest{
    0x9d, 0xd2, 0xfb, 0xd2, 0x70, 0xfe, 0xaa, 0x1f, 0xbe, 0xf2, 0xd4, 0xf6, 0x34, 0xd7, 0x88, 0x7c,
    0x9c, 0x50, 0x6e, 0x3b, 0xde, 0x14, 0x0f, 0x8e, 0x73, 0x51, 0xc8, 0x94, 0x4e, 0x8f, 0xd2, 0x35,
};
...
CompiledChatTemplate CompiledChatTemplate::resolve(std::string_view source) {
    ...
    if (digest == kThinkingToggleTemplateDigest || digest == kQwen35ChatTemplateDigest ||
        digest == kQwen35NonThinkingDefaultChatTemplateDigest ||
        digest == kOrnith15ChatTemplateDigest) {
