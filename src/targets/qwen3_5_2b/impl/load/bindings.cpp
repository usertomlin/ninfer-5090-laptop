...
DensePostMixerPayload load_mlp(const MlpPlan& plan,
    ...
    out.gate_up = materialized_weight(materialized, plan.gate_up, 12288, 2048);
    out.down    = materialized_weight(materialized, plan.down, 2048, 6144);
    return out;
}
...
load_attention_projection(const FullAttentionPlan& plan,
    ...
    return SplitAttentionProjectionPayload{
        .query_key  = materialized_weight(materialized, split->query_key, 2560, 2048),
        .gate_value = materialized_weight(materialized, split->gate_value, 2560, 2048),
    };
}
...
load_gdn_input_projection(const GdnPlan& plan, const artifact::MaterializedArtifact& materialized) {
    ...
    return SplitGdnInputProjectionPayload{
        .query_key = materialized_weight(materialized, split->query_key, 4096, 2048),
        .value_z   = materialized_weight(materialized, split->value_z, 4096, 2048),
    };
}
        ...
        if (target.is_full_attention) {
            target.attention.projection = SplitAttentionProjectionPlan{
                .query_key  = bind_weight(binder, prefix + "attention/query_key",
                                          NumericFormat::Q4G64_F16S, {2560, 2048}),
                .gate_value = bind_weight(binder, prefix + "attention/gate_value",
                                          NumericFormat::Q5G64_F16S, {2560, 2048}),
            };
            ...
            target.attention.output = bind_weight(binder, prefix + "attention/output",
                                                  NumericFormat::Q5G64_F16S, {2048, 2048});
        } else {
            ...
            target.gdn.output =
                bind_weight(binder, prefix + "gdn/output", NumericFormat::Q5G64_F16S, {2048, 2048});
        }
        target.post_attention_norm = artifact::bind_device_tensor(
            binder, prefix + "post_attention_norm", NumericFormat::BF16, {2048});
        target.mlp.gate_up =
            bind_weight(binder, prefix + "mlp/gate_up", NumericFormat::Q4G64_F16S, {12288, 2048});
        target.mlp.down =
            bind_weight(binder, prefix + "mlp/down", NumericFormat::Q5G64_F16S, {2048, 6144});
    }
}
...
RuntimeModelView build_runtime(BindingPlan const& plan,
    ...
    view.token_embedding = materialized_weight(materialized, plan.token_embedding, 248320, 2048);
    ...
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (source.is_full_attention) {
            ...
            target.input_norm            = artifact::materialized_tensor(
                materialized, source.input_norm, NumericFormat::BF16, {2048});
            ...
            target.output = materialized_weight(materialized, source.attention.output, 2048, 2048);
            target.post_attention_norm = artifact::materialized_tensor(
                materialized, source.post_attention_norm, NumericFormat::BF16, {2048});
            ...
            target.projection.control_projection = SplitGdnControlProjectionPayload{
                .a_projection = artifact::materialized_weight(
                    materialized, source.gdn.a_projection, NumericFormat::BF16, 16, 2048),
                .b_projection = artifact::materialized_weight(
                    materialized, source.gdn.b_projection, NumericFormat::BF16, 16, 2048),
            };
            ...
            target.norm = artifact::materialized_tensor(materialized, source.gdn.norm,
                                                        NumericFormat::BF16, {128});
            target.output = materialized_weight(materialized, source.gdn.output, 2048, 2048);
            target.post_attention_norm = artifact::materialized_tensor(
                materialized, source.post_attention_norm, NumericFormat::BF16, {2048});
            target.post_mixer = load_mlp(source.mlp, materialized);
        }
    ...
    view.final_norm  = artifact::materialized_tensor(materialized, plan.final_norm,
                                                     NumericFormat::BF16, {2048});
    view.output_head = materialized_weight(materialized, plan.output_head, 248320, 2048);
    ...
    if (plan.features.optimized_proposal()) {
        ...
        proposal.head      = artifact::materialized_weight(
            materialized, plan.draft_head, NumericFormat::Q4G64_F16S, 131072, 2048);
    ...
    if (plan.features.mtp()) {
        ...
        mtp.input_projection = artifact::materialized_weight(
            materialized, plan.mtp.input_projection, NumericFormat::W8G32_F16S, 2048, 4096);
        ...
        mtp.attention.packed = artifact::materialized_weight(
            materialized, plan.mtp.query_key_gate_value, NumericFormat::W8G32_F16S, 5120, 2048);
        ...
        mtp.output = artifact::materialized_weight(
            materialized, plan.mtp.output, NumericFormat::W8G32_F16S, 2048, 2048);
        mtp.post_attention_norm = artifact::materialized_tensor(
            materialized, plan.mtp.post_attention_norm, NumericFormat::BF16, {2048});
    ...
    if (plan.features.vision) {
        ...
        vision.merger_fc2 = artifact::materialized_weight(
            materialized, plan.vision_merger_fc2, NumericFormat::W8G32_F16S, 2048, 4096);
        vision.merger_fc2_bias = artifact::materialized_tensor(
            materialized, plan.vision_merger_fc2_bias, NumericFormat::BF16, {2048});
    }
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    if (weights_profile != WeightsProfile::GroupwiseInt) {
        throw std::invalid_argument("qwen3_5_2b: only GroupwiseInt is supported");
    }
    ...
    out.mtp.query_key_gate_value = bind_mtp("mtp/layer/attention/query_key_gate_value",
                                            NumericFormat::W8G32_F16S, {5120, 2048});
    ...
    out.mtp.output =
        bind_mtp("mtp/layer/attention/output", NumericFormat::W8G32_F16S, {2048, 2048});
    out.mtp.post_attention_norm =
        bind_mtp("mtp/layer/post_attention_norm", NumericFormat::BF16, {2048});
    out.mtp.mlp.gate_up = WeightPlan{
        .object = bind_mtp("mtp/layer/mlp/gate_up", NumericFormat::W8G32_F16S, {12288, 2048}),
        .format = NumericFormat::W8G32_F16S};
    out.mtp.mlp.down = WeightPlan{
        .object = bind_mtp("mtp/layer/mlp/down", NumericFormat::W8G32_F16S, {2048, 6144}),
        .format = NumericFormat::W8G32_F16S};
    ...
    out.vision_merger_fc2 = artifact::bind_tensor(
        binder, "vision/merger/fc2", NumericFormat::W8G32_F16S, {2048, 4096}, vision_placement);
