"""Pinned official Qwen3.5-0.8B frontend resources used by artifact conversion.

This module owns only the checkpoint-invariant resource profile for the
Qwen3.5-0.8B target.  The Qwen3.5-0.8B release ships no `generation_config.json`;
...
def load_official_resources(
    ...
    if spec_names != expected_names:
        raise ValueError(
            ...
            f"Qwen3.5-0.8B profile: expected {expected_names!r}, got {spec_names!r}"
        )
    disk_specs = [
