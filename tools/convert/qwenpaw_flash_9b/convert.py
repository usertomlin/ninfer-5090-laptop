"""Convert the QwenPaw-Flash-9B checkpoint into one complete artifact.

QwenPaw-Flash-9B is architecturally identical to Qwen3.5-9B but ships no MTP
weights.  The converter fills zero tensors for MTP objects so the artifact
object list matches what the C++ engine binding expects.

Canonical invocation::

    python -m tools.convert.qwenpaw_flash_9b.convert \\
      --model /path/to/QwenPaw-Flash-9B \\
      --out out/qwenpaw_flash_9b.ninfer
"""
...
def validate_config(config: Mapping[str, object]) -> dict[str, object]:
    """Validate the registered checkpoint dimensions and summarize them."""
    ...
    _check_members("text_config", text, _TEXT_CONFIG_KEYS)
    ...
    return {
        ...
        "text": {name: text[name] for name in _TEXT_CONFIG_KEYS},
        "layer_types": {
...
def _make_zero_tensor(spec: inventory.TensorSpec) -> torch.Tensor:
    if spec.format in ("BF16",):
        return torch.zeros(spec.shape, dtype=torch.bfloat16)
    if spec.format in ("FP32",):
        return torch.zeros(spec.shape, dtype=torch.float32)
    if spec.format in ("I32",):
...
def materialize_tensor(
    ...
    if spec.name.startswith("mtp/"):
        return _make_zero_tensor(spec)
    ...
    tensor = recipe.materialize_recipe(
        recipe.SOURCE_RECIPES_BY_NAME[spec.name],
        reader,
        derived,
    )
